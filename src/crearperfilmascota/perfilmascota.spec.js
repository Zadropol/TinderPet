import { usarRepositorioTest, crearperfil } from "./perfilmascota.js";

describe("Crear Perfil", () => {
    beforeEach(() => {
    usarRepositorioTest(true); // activa repositorio de prueba vacío
  });
...
//   });
  ...
  // });
    ...
    it("Debería poder ingresar y mostrar una imagen de la mascota", async () => {
      const imageUrl = "https://www.adnradio.cl/2024/08/22/que-significa-firulais-el-popular-nombre-que-tienen-muchos-perros-en-el-mundo/";

      const perfil = await crearperfil("Firulais", 3, "Pitbull", imageUrl);

      const { perfilAHTML } = await import("./perfilesenHTML.js");
      const html = perfilAHTML(perfil);
      expect(html).toContain("<h3>Firulais</h3>");
      expect(html).toContain("<p>Edad: 3 años</p>");
      expect(html).toContain("<p>Raza: Pitbull</p>");
      expect(html).toContain(`<img src="${imageUrl}" alt="Firulais">`);
    });
